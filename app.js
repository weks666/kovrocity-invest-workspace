...
const workspaceNavigation=createWorkspaceNavigation({key:'kovrocity-invest',title:'KovroCity Invest',actions:[
...
function applyInvestTheme(value, { persist = !runtimeAdapter } = {}) {
  ...
  if (persist) { try { localStorage.setItem('kovrocity-invest:appearance',theme); } catch { /* Device preference is optional. */ } }
}
try { applyInvestTheme(runtimeAdapter ? 'invest' : localStorage.getItem('kovrocity-invest:appearance'), { persist: false }); } catch { applyInvestTheme('invest', { persist: false }); }
...
function renderRuntimeTruth(data) {
  ...
  const displayName = identity.displayName || identity.name || 'KovroCity';
